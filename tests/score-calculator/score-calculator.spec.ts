...
// Reference: Student sheet, TC-STU-072 "Verify that Student completes the
// Score Calculator (V2)". The sheet documents this as Blocked - Listening,
// Reading and Writing all completed successfully, but Speaking couldn't be
// tested because the automation environment had no microphone. Confirmed
// live this suite's environment (launchOptions fake-device flags + granted
// mic permission in playwright.config.ts) doesn't have that limitation, so
// this test goes all the way through to a completed score report.
test.describe('Score Calculator', () => {
  test('TC-STU-072: Student completes the Score Calculator (V2) and receives a score report', async ({ page }) => {
