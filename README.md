...
| TC-STU-064 (was 069) | Smoke | Paid student watches a recorded class in the Watched tab (tries a pool of fallback accounts — see Known findings) | `tests/online-classes/recorded-classes.spec.ts` |
...
- **TC-STU-064 (watch a recorded class) tries a pool of fallback accounts**: each account's recorded-class list is small and finite, so a fixture eventually runs out of anything left to mark "Watched" (confirmed live 2026-09-10: the original fixture hit 3-for-3 watched). Rather than failing outright the moment that happens, the test tries every account in `POWER_TIER_ACCOUNT_POOL` (`test-data/registrationData.ts`) in turn and uses the first one with an unwatched class, only skipping if every account in the pool is exhausted. The pool reads `FIXTURE_POWER_TIER_EMAIL`/`PASSWORD` plus any numbered fallbacks (`_EMAIL_2`/`_PASSWORD_2`, `_3`, ...) from `.env` — none are required, so add one whenever the current pool runs dry rather than needing to plan a fixed number upfront. Every other spec that logs into `powerTier` is unaffected and always uses just the first account.
